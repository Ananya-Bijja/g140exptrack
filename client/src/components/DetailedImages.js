...
const DetailedImages = () => {
  ...
  return (
    <div>
      ...
      {/* Display session selection buttons */}
      {detailedData.map((user) => (
        <div key={user.username}>
          <h2>User: {user.username}</h2>
          <div>
            {user.sessions.map((session) => (
              <button key={session.gameSessionId} onClick={() => handleSessionSelection(session.gameSessionId)}>
                View Session {session.gameSessionId}
              </button>
            ))}
          </div>
        </div>
      ...
      {selectedSessionId && filteredData.map((user) => (
        user.sessions.map((session) => (
          session.gameSessionId === selectedSessionId && (
            <div key={session.gameSessionId}>
              <h3>Game Session ID: {session.gameSessionId}</h3>

              {session.imagesWithEmotions.map((image) => (
                <div key={image.imagePath}>
                  {/* Image and captured time displayed outside the table */}
                  <div>
                    <img
                      src={`http://localhost:5000/uploads/${image.imagePath}`}
                      alt={image.imagePath}
